...
function testbridge.test_print_tsfi_for_sfr_rows()
   local expectedresult_all = [[\midrule\relax\hypertarget{fcs_ckm.1}{\sfr{fcs_ckm.1}} & \tsfilink{ls.lan.tls}&\ndocsupabbrev&Key negotiation for TLS\\&\tsfilink{ls.wan.ipsec}&\ndocsupabbrev&Key negotiation for VPN\\\midrule\relax\hypertarget{fcs_ckm.2/ike}{\sfr{fcs_ckm.2/ike}} & \tsfilink{ls.wan.ipsec}&\ndocsupabbrev&Key distribution for VPN\\\midrule\relax\hypertarget{fcs_ckm.2/tls}{\sfr{fcs_ckm.2/tls}} & \tsfilink{ls.lan.tls}&\ndocsupabbrev&Key distribution for TLS\\\midrule\relax\hypertarget{fcs_ckm.4}{\sfr{fcs_ckm.4}} & \tsfilink{ls.lan.tls}&\ndocsupabbrev&Terminate TLS connections to LAN\\&\tsfilink{ls.wan.ipsec}&\ndocsupabbrev&Terminate IPSEC connections to WAN\\\midrule\relax\hypertarget{fcs_cop.1/hash}{\sfr{fcs_cop.1/hash}} & \tsfilink{ls.wan.ipsec}&\ndocenfabbrev&IPSec hash operations\\&\tsfilink{ls.lan.tls}&\ndocenfabbrev&TLS hash operations\\\midrule\relax\hypertarget{fcs_cop.1/hmac}{\sfr{fcs_cop.1/hmac}} & \tsfilink{ls.wan.ipsec}&\ndocenfabbrev&IPSec HMAC operations\\&\tsfilink{ls.lan.tls}&\ndocenfabbrev&TLS HMAC operations\\\midrule\relax\hypertarget{fcs_cop.1/tls.aes}{\sfr{fcs_cop.1/tls.aes}} & \tsfilink{ls.lan.tls}&\ndocenfabbrev&TLS connections\\\midrule\relax\hypertarget{fcs_cop.1/tls.auth}{\sfr{fcs_cop.1/tls.auth}} & \tsfilink{ls.lan.tls}&\ndocenfabbrev&TLS connections\\\midrule\relax\hypertarget{fcs_rng.1/hashdrbg}{\sfr{fcs_rng.1/hashdrbg}} & \tsfilink{ls.lan.tls}&\ndocsupabbrev&TLS connections\\&\tsfilink{ls.wan.ipsec}&\ndocsupabbrev&Key negotiation for VPN\\\midrule\relax\hypertarget{fdp_rip.1}{\sfr{fdp_rip.1}} & \tsfilink{no_tsfi}&\ndocenfabbrev&Not accessible via TSFI\\\midrule\relax\hypertarget{fpt_tdc.1/tls.zert}{\sfr{fpt_tdc.1/tls.zert}} & \tsfilink{ls.lan.tls}&\ndocenfabbrev&Validate TLS certificate\\\midrule\relax\hypertarget{fpt_tdc.1/zert}{\sfr{fpt_tdc.1/zert}} & \tsfilink{ls.wan.ipsec}&\ndocenfabbrev&Validate VPN certificate\\\midrule\relax\hypertarget{fpt_stm.1}{\sfr{fpt_stm.1}} & \tsfilink{ls.wan.ntp}&\ndocenfabbrev&Access to time service\\\midrule\relax\hypertarget{fpt_tst.1}{\sfr{fpt_tst.1}} & \tsfilink{ls.lan.httpmgmt}&\ndocenfabbrev&Call self-test\\\midrule\relax\hypertarget{ftp_itc.1/tls}{\sfr{ftp_itc.1/tls}} & \tsfilink{ls.lan.tls}&\ndocenfabbrev&Secure connection to management interface\\\midrule\relax\hypertarget{ftp_itc.1/vpn}{\sfr{ftp_itc.1/vpn}} & \tsfilink{ls.wan.ipsec}&\ndocenfabbrev&Secure IPSec tunnel\\\midrule\relax\hypertarget{ftp_trp.1/admin}{\sfr{ftp_trp.1/admin}} & \tsfilink{ls.lan.httpmgmt}&\ndocenfabbrev&Connection to management interface\\]]
...
function testbridge.test_print_subsys_to_sfr_table()
   local expectedresult = [[\begin{enfsfrsubsystable}Enforcing~SFR & \ndocpurpose \\\midrule\relax\sfrlinknoindex{fcs_ckm.2/tls} & Connection establishment, key distribution for TLS\\\sfrlinknoindex{fcs_cop.1/tls.aes} & Provide AES for TLS\\\sfrlinknoindex{fcs_cop.1/tls.auth} & Authentication of TLS peer\\\sfrlinknoindex{fpt_tdc.1/tls.zert} & Certificate validation\\\sfrlinknoindex{ftp_itc.1/tls} & Establish/terminate TLS connections\\\end{enfsfrsubsystable}]]
...
function testbridge.test_getSecfuncText()
   bridge_cc_core.getSecfuncText("sf.cryptographicservices", tex.expected([[Cryptographic Services]]))
end

function testbridge.test_getObjective()
   bridge_cc_core.getObjective("o.timeservice", tex.expected([[O.Time\_Service]]))
end

function testbridge.test_getObjectiveText()
   bridge_cc_core.getObjectiveText("o.timeservice", tex.expected([[Using a secure time service]]))
end
...
function testbridge.test_getSubjobjText()
   bridge_cc_core.getSubjobjText("s_admin", tex.expected([[Subject that acts for an administrator.]]))
end

function testbridge.test_getTsfi()
   bridge_cc_core.getTsfi("ls.lan.tls", tex.expected([[LI.LAN.TLS]]))
end
